...
let getBossZhipinJob = async (sifiConds = sifiConfig, selectorList = siteSelectorList) => {
    ...
    const browser = await (puppeteer.launch({
        headless: true, //要看演示可以使用 false  // 设为ture好像要报错:  ProtocolError: Protocol error (Runtime.callFunctionOn): Execution context was destroyed.
        ...
        try {
            ...
            const jobList = await page.evaluate((sel) => {
                ...
                const item = JobInfo.map(v => {
                    ...
                    const jobArea = $(v).find('div.info-primary span.job-area').text().trim();
